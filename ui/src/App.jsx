...
import { ArrowsOutSimple, CalendarBlank, CaretDown, Check, Copy, Crosshair, FilePdf, Funnel, ImageSquare, MagnifyingGlass, MapPin, SquaresFour, X } from "@phosphor-icons/react";
...
function normalizeScene(scene,index){
 ...
 return{id:scene.id,name:scene.name,provider:scene.path?.split("/")[0]||scene.platform||"Unknown",sensor:scene.sensor||"Unknown",level:scene.processing_level||"Unknown",acquired:scene.acquired_at?new Date(scene.acquired_at).toLocaleString("en-GB",{timeZone:"UTC"})+" UTC":"Unknown",file:previewName?publicAsset(`previews/${previewName}`):"",size:`${(scene.size_bytes/1073741824).toFixed(2)} GB`,crs:scene.crs||"Unknown",dimensions:scene.width&&scene.height?`${scene.width.toLocaleString()} × ${scene.height.toLocaleString()}`:"Unknown",path:scene.path,footprint:scene.footprint,bounds:scene.bounds,x:10+((index*17)%66),y:12+((index*23)%52),rotation:-8+(index%5)*4};
}

function footprintLatLngs(scene){
 const ring=scene.footprint?.coordinates?.[0];
 if(!ring?.length)return null;
 try{return ring.map(([x,y])=>{const [lon,lat]=Math.abs(x)<=180&&Math.abs(y)<=90?[x,y]:proj4(scene.crs,"EPSG:4326",[x,y]);return[lat,lon]}).filter(([lat,lon])=>Number.isFinite(lat)&&Number.isFinite(lon)&&Math.abs(lat)<=90&&Math.abs(lon)<=180)}catch{return null}
}

function SceneMap({scenes,active,onSelect,onPreview}){
 ...
 return <div ref={elementRef} className="map-stage" aria-label={`OpenStreetMap with ${scenes.length} satellite scene footprints`}/>;
}
...
export function App(){
 ...
 const[query,setQuery]=useState(""),[provider,setProvider]=useState("All"),[sensor,setSensor]=useState("All"),[level,setLevel]=useState("All"),[active,setActive]=useState(demoScenes[12]),[previewScene,setPreviewScene]=useState(null),[selected,setSelected]=useState(new Set([demoScenes[12].id])),[tab,setTab]=useState("Overview"),[onlyVisible,setOnlyVisible]=useState(false),[copied,setCopied]=useState(false),[exporting,setExporting]=useState(false),[pdfReady,setPdfReady]=useState(null);
 useEffect(()=>{fetch(publicAsset("data/catalog.json"),{cache:"no-store"}).then(response=>response.ok?response.json():Promise.reject()).then(data=>{const next=(data.scenes||[]).filter(scene=>scene.preview).map(normalizeScene);if(next.length){setCatalogScenes(next);setActive(next[0]);setSelected(new Set([next[0].id]))}}).catch(()=>{})},[]);
 ...
 return <main className="app-shell">
  ...
  <section className="workspace"><SceneMap scenes={filtered} active={active} onSelect={setActive} onPreview={setPreviewScene}/><div className="results-head"><strong>{filtered.length} scenes</strong><span>Sorted by acquisition date <CaretDown size={13}/></span></div><div className="scene-strip">{filtered.map(s=><article key={s.id} className={`scene-card ${active?.id===s.id?"active":""}`} onClick={()=>setActive(s)}><button className={`select-dot ${selected.has(s.id)?"selected":""}`} onClick={e=>{e.stopPropagation();toggle(s.id)}} aria-label="Select scene">{selected.has(s.id)&&<Check size={12} weight="bold"/>}</button><button className="card-preview-button" onClick={e=>{e.stopPropagation();setPreviewScene(s)}} aria-label={`View large preview of ${s.id}`}><ImageSquare size={16}/></button><img src={s.file} alt={`Preview of ${s.id}`} loading="lazy"/><div><strong>{s.id}</strong><span>{s.sensor} · {s.level}</span><span>{s.acquired.split(" UTC")[0]}</span></div></article>)}</div></section>
  <aside className="details">{active?<><div className="detail-title"><div><strong>{active.id}</strong><span>{active.provider} · {active.level}</span></div><button onClick={()=>setActive(null)} aria-label="Close details"><X/></button></div><img className="hero-preview" src={active.file} alt={`Selected scene ${active.id}`}/><div className="tabs">{["Overview","Metadata","Location"].map(n=><button key={n} className={tab===n?"active":""} onClick={()=>setTab(n)}>{n}</button>)}</div>{tab==="Overview"&&<dl className="metadata"><div><dt>Scene ID</dt><dd>{active.id}</dd></div><div><dt>Provider</dt><dd>{active.provider}</dd></div><div><dt>Sensor</dt><dd>{active.sensor}</dd></div><div><dt>Acquisition</dt><dd>{active.acquired}</dd></div><div><dt>Processing level</dt><dd>{active.level}</dd></div><div><dt>File size</dt><dd>{active.size}</dd></div></dl>}{tab==="Metadata"&&<dl className="metadata"><div><dt>Format</dt><dd>GeoTIFF</dd></div><div><dt>CRS</dt><dd>{active.crs}</dd></div><div><dt>Dimensions</dt><dd>{active.dimensions}</dd></div><div><dt>Preview</dt><dd>4096 px JPEG</dd></div></dl>}{tab==="Location"&&<div className="location-panel"><MapPin size={26} weight="duotone"/><div><strong>Central Kalimantan</strong><span>Footprint shown on map</span></div></div>}<div className="source-path"><span>Source path</span><p>{active.path}</p><button onClick={copyPath}>{copied?<Check/>:<Copy/>} {copied?"Copied":"Copy path"}</button></div><button className={`primary-action ${selected.has(active.id)?"selected":""}`} onClick={()=>toggle(active.id)}><SquaresFour size={18}/>{selected.has(active.id)?"Remove from report":"Add to report"}</button><div className="selection-summary"><span>{selected.size} selected</span><button onClick={()=>exportReport(true)} disabled={!selected.size||exporting}><FilePdf/> {exporting?"Creating…":"Export selected"}</button></div></>:<div className="details-empty"><Crosshair size={36} weight="duotone"/><strong>No scene selected</strong><span>Select a filtered footprint or scene card to inspect its information.</span></div>}</aside>
